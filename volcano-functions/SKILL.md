...
- **Use `console.log`** for debug output, then fetch it with the CLI — `volcano functions logs <name> --type runtime` (`--type build` for deploy/packaging errors, `--follow` to tail while re-invoking) — before reaching for the Volcano dashboard, which is a secondary view for cloud projects only.
