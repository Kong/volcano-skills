...
4. Route persistent data access through the Volcano Database query-builder pattern. Only when a specific query-builder gap (joins/aggregations/multi-statement transactions) is provably impossible otherwise, fall back to direct Postgres access as a discouraged, narrowly-scoped last resort (see `volcano_database`'s "Direct Postgres Access" section — requires the `application_name=volcano_user_access:{user_id}` rewrite to stay RLS-safe).
