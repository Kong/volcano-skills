...
4. Route persistent data access through the Volcano Database query-builder pattern, except joins/aggregations/multi-statement transactions/ORMs, which use direct Postgres access instead (see `volcano_database`'s "Direct Postgres Access" section — requires the `application_name=volcano_user_access:{user_id}` rewrite to stay RLS-safe).
