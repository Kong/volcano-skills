...
3. **Check logs for the specific resource**: `volcano functions logs <name> --type
   build|runtime` (add `--follow` to tail while re-invoking).
...
session to debug from here. The only legitimate browser step anywhere in this workflow
is the human's device-code approval during `volcano login`/`volcano signup` — never a
diagnostic detour.
